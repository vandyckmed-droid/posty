...
PINNED = C.curated_universe() is not None      # list is the universe; no screening
...
for sym, meta in cand.items():
    ...
    med = st.median([b['close'] * b['volume'] for b in bars[-C.LIQ_WINDOW:]]) if bars else 0
    if not PINNED:
        if len(bars) < need:
    ...
    rows.append({'symbol': sym, 'name': meta['name'], 'medDollarVol': med,
                 ...
                 'bars': len(bars), 'cat': meta.get('cat', ''),
